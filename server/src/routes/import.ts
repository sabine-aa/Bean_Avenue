...
const yesNo = (v: unknown, dflt: boolean): boolean => {
  const t = low(v);
...
const parseDate = (v: unknown): { ok: boolean; date: Date | null } => {
  ...
  return Number.isNaN(d.getTime()) ? { ok: false, date: null } : { ok: true, date: d };
};
...
async function buildContext(payload: Payload): Promise<Ctx> {
  ...
  for (const r of payload.stock ?? []) {
    const n = low(r.itemName);
    if (n) stockUnits.set(n, low(r.unit));
  }
...
function validateAll(payload: Payload, ctx: Ctx): ValResult {
  ...
  const stock = (payload.stock ?? [])
    .filter((r) => s(r.itemName))
    .map((r, i) =>
      annotate(r, i, "A - Current Stock", () => {
        const errs: string[] = [];
        const warns: string[] = [];
  ...
  const recipes = (payload.recipes ?? [])
    .filter((r) => s(r.menuItem))
    .map((r, i) =>
      annotate(r, i, "B - Recipes", () => {
        const errs: string[] = [];
        const warns: string[] = [];
        ...
        if (s(r.size) && menu) {
          ...
          else if (!menu.sizes.some((x) => x.toLowerCase() === low(r.size)))
            errs.push(`Size "${s(r.size)}" is not a size of "${s(r.menuItem)}" (${menu.sizes.join(", ")}).`);
        }
        // add-on
  ...
  const products = (payload.products ?? [])
    .filter((r) => s(r.productName))
    .map((r, i) =>
      annotate(r, i, "C - Products", () => {
        const errs: string[] = [];
        const warns: string[] = [];
        ...
        else if (type === "cafe" || type === "café") {
          if (!ctx.menu.has(low(r.productName))) errs.push(`Cafe product "${s(r.productName)}" not found in the menu — create it in Menu Manager first.`);
        } else if (type === "retail") {
          if (!ctx.shop.has(low(r.productName))) errs.push(`Retail product "${s(r.productName)}" not found in Shop Products — create it there first.`);
        }
        for (const [k, label] of [
          ["currentQty", "Current Qty"],
          ["minQty", "Min Qty"],
  ...
  // ---- D: hanson ----
  const hanson = (payload.hanson ?? [])
    .filter((r) => s(r.doughnut))
    .map((r, i) =>
      annotate(r, i, "D - Hanson Daily", () => {
        const errs: string[] = [];
        const warns: string[] = [];
        ...
        if (h && s(r.category) && h.subcategory && low(r.category) !== h.subcategory.toLowerCase())
          warns.push(`Category "${s(r.category)}" differs from the catalogue (${h.subcategory}).`);
        return { errs, warns };
      }),
    );
...
importRouter.post("/commit", async (req, res) => {
  ...
  for (const r of v.tabs.stock) {
    ...
    const data = {
      ...
      costPerUnit: num(r.costPerUnit) ?? 0,
      supplier: s(r.supplier) || null,
      expiryDate: parseDate(r.expiryDate).date,
      trackExpiry: yesNo(r.trackExpiry, false),
      batchLot: s(r.batchLot) || null,
      storageLocation: s(r.storageLocation) || null,
    ...
    await prisma.inventoryMovement.create({
      data: { inventoryItemId: item.id, delta, balance: qty as number, type: "COUNT", reason: "Excel import", staffName: actor.actorName },
    });
    results.stock++;
  }
  ...
  for (const r of v.tabs.products) {
    ...
    if (type === "cafe" || type === "café") {
      ...
      await prisma.menuItem.update({
        where: { id: mid },
        data: { trackStock: true, stockQty: qty, lowStockAt: num(r.minQty) ?? 0, costPrice: num(r.cost) ?? 0 },
      });
      await prisma.stockMovement.create({
        data: { menuItemId: mid, delta: qty - (before?.stockQty ?? 0), balance: qty, type: "COUNT", reason: "Excel import", staffName: actor.actorName },
      });
      results.products++;
    } else if (type === "retail") {
      ...
      await prisma.shopStockMovement.create({
        data: { productId: pid, delta: qty - (before?.quantity ?? 0), balance: qty, type: "COUNT", reason: "Excel import", staffName: actor.actorName },
      });
      results.products++;
    } else skip();
  }
  ...
  for (const r of v.tabs.hanson) {
    ...
    await prisma.menuItem.update({
      where: { id: h.id },
      data: { availableToday: yesNo(r.availableToday, true), ...(cost != null && !Number.isNaN(cost) ? { costPrice: cost } : {}) },
    });
    results.hanson++;
  }
  ...
  if (results.stock)
    await audit(actor, {
      section: "Inventory",
      action: "import_stock",
      description: `Excel import: ${results.stock} inventory item(s)`,
      entity: "Import",
      newValue: { count: results.stock },
    });
  if (results.recipes)
    await audit(actor, {
      section: "Inventory",
      action: "import_recipes",
      description: `Excel import: ${results.recipes} recipe line(s) across ${byMenu.size} menu item(s)`,
      entity: "Import",
      newValue: { count: results.recipes, menuItems: byMenu.size },
    });
  if (results.products)
    await audit(actor, {
      section: "Inventory",
      action: "import_products",
      description: `Excel import: ${results.products} product stock row(s)`,
      entity: "Import",
      newValue: { count: results.products },
    });
  if (results.hanson)
    await audit(actor, {
      section: "Hanson",
      action: "import_hanson",
      description: `Excel import: ${results.hanson} Hanson daily row(s)`,
      entity: "Import",
      newValue: { count: results.hanson },
    });
