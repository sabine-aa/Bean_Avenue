  ...
  return (
    <div className="card-lift flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm">
      <Link to={`/menu/${item.id}`} className="relative block">
        ...
        />
        {item.isBestSeller && (
          <span className="absolute left-2 top-2 rounded-full bg-terracotta px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide text-cream shadow-sm">
            ★ Best Seller
          </span>
        )}
      </Link>
      <div className="flex flex-1 flex-col p-3 sm:p-4">
        <div className="flex items-start justify-between gap-2">
          ...
          <span className="whitespace-nowrap font-semibold text-terracotta">
            {hasOptions ? `From ${money(item.price)}` : money(item.price)}
          </span>
        </div>
        <p className="mt-0.5 text-xs font-medium uppercase tracking-wide text-charcoal/40">{item.category}</p>
        <p className="mt-1 line-clamp-2 flex-1 text-sm text-charcoal/70">{item.description}</p>
        {item.nutrition?.kcal != null && (
          <p className="mb-1 text-xs font-semibold text-sage-dark">
