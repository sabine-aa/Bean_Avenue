    ...
    <div className="mx-auto max-w-2xl px-4 py-12 sm:py-16">
      ...
      {awaitingPayment && order && (
        <div className="mt-6 rounded-2xl border-2 border-amber-300 bg-amber-50 p-5 text-center">
          ...
          <button onClick={() => setPayOpen(true)} className="btn-3d mt-3 rounded-full bg-terracotta px-6 py-3 font-semibold text-cream">
        ...
        <>
          ...
          <div className="mt-8 rounded-2xl bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p className="font-display text-xl font-bold text-espresso">{meta.icon} {meta.label}</p>
              </div>
              <div className="text-right">
                ...
                {estimate && <p className="mt-1 text-xs text-charcoal/60">{isDelivery ? "Delivery" : "Pickup"}: {estimate}</p>}
              </div>
            </div>
            ...
            {order.driverName && isDelivery && status !== "CANCELLED" && (
              <p className="mt-4 text-center text-sm text-charcoal/70">🛵 {order.driverName} is handling your delivery.</p>
            )}
            {!["COMPLETED", "DELIVERED", "CANCELLED"].includes(status) && (
              <p className="mt-4 text-center text-xs text-charcoal/40">This page updates automatically as your order progresses.</p>
            )}
          </div>
          ...
          <div className="mt-6 rounded-2xl bg-white p-6 text-left shadow-sm">
            {isDelivery ? (
              <div className="text-sm">
                <p className="font-display font-bold text-espresso">🛵 Delivery</p>
                {order.zoneName && <p className="mt-1 text-charcoal/70">Zone: {order.zoneName} · {order.estimatedDelivery}</p>}
                ...
                {order.deliveryInstructions && <p className="mt-1 text-charcoal/60">📝 {order.deliveryInstructions}</p>}
                <a
                  href={order.lat != null && order.lng != null ? mapsLinkFromCoords(order.lat, order.lng) : mapsLinkFromText([order.building, order.area].filter(Boolean).join(" "))}
                  target="_blank"
                  rel="noreferrer"
              ...
              <div className="text-sm">
                <p className="font-display font-bold text-espresso">🏪 Pickup</p>
                <p className="mt-1 text-charcoal/70">{order.pickupTime === "ASAP" ? "As soon as possible" : order.pickupTime} · {ADDRESS}</p>
              </div>
            )}
          </div>
          ...
          <div className="mt-6 rounded-2xl bg-white p-6 text-left shadow-sm">
            ...
            <div className="mt-4 space-y-1.5 border-t border-oat pt-3 text-sm">
              ...
              {order.discount > 0 && <SummaryRow label={`Discount${order.promoCode ? ` (${order.promoCode})` : ""}`} value={`−${money(order.discount)}`} accent />}
            ...
            <div className="mt-4 rounded-xl bg-oat/60 p-3 text-sm">
              <p><strong>Payment:</strong> {PAYMENT_METHOD_LABEL[order.paymentMethod]} · {paymentStatusMeta(order.paymentStatus).label}</p>
              {order.beansEarned > 0 && (
                <p className="mt-1 text-sage-dark">
                  🫘 {paid ? "You earned" : "You'll earn"} <strong>{order.beansEarned} beans</strong>{paid ? " on this order!" : " once this order is complete."}
                </p>
              )}
            </div>
          </div>
      ...
      <div className="mt-8 flex flex-wrap justify-center gap-3">
        <Link to="/account?tab=orders" className="btn-3d rounded-full bg-espresso px-6 py-2.5 font-semibold text-cream">
          Track in My Orders
        </Link>
        <Link to="/menu" className="rounded-full border border-oat bg-white px-6 py-2.5 font-semibold text-espresso">
          Back to Menu
        </Link>
        {canCancel && (
          <button onClick={cancelOrder} disabled={cancelling} className="rounded-full border border-terracotta/40 bg-white px-6 py-2.5 font-semibold text-terracotta-dark disabled:opacity-60">
            {cancelling ? "Cancelling…" : "Cancel order"}
          </button>
        )}
        <a href={WHATSAPP_URL} target="_blank" rel="noreferrer" className="rounded-full border border-oat bg-white px-6 py-2.5 font-semibold text-espresso">
          💬 WhatsApp
        </a>
      </div>
