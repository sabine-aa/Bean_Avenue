...
const expectedOf = (s: Shift) => (s.expectedCash != null ? s.expectedCash : s.openingFloat + s.cashSales + s.cashPayIns - s.cashPayOuts);

function Figure({ label, value, tone }: { label: string; value: string; tone?: string }) {
  return (
    <div>
      <p className="text-charcoal/45 text-[11px] font-medium tracking-wide uppercase">{label}</p>
...
export function AdminShifts() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="font-display text-espresso text-3xl font-bold">Shifts &amp; cash drawer</h1>
        <p className="text-charcoal/60 mt-1 text-sm">Register Z-reports — opening float, sales, and the counted-vs-expected cash for every shift.</p>
      </div>

      {shifts.length === 0 && <p className="text-charcoal/60 rounded-2xl bg-white p-6 text-center shadow-sm">No shifts recorded yet.</p>}
        ...
        {shifts.map((s) => {
          ...
          return (
            <section key={s.id} className="rounded-2xl bg-white p-5 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <p className="text-espresso flex items-center gap-2 font-semibold">
                    ...
                    <span
                      className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${s.status === "OPEN" ? "bg-sage/25 text-sage-dark" : "bg-oat text-charcoal/60"}`}
                    >
                  ...
                  <p className="text-charcoal/50 mt-0.5 text-xs">
                    ...
                    {s.closedAt ? ` → ${formatDateTime(s.closedAt)}` : " → in progress"}
                  </p>
                </div>
                <div className="text-right">
                  ...
                  <p className="font-display text-espresso text-lg font-bold">{money(s.cashSales + s.cardSales)}</p>
                </div>
              </div>
              ...
              {s.note && <p className="text-charcoal/60 mt-2 text-sm">📝 {s.note}</p>}

              {s.movements.length > 0 && (
                <div className="mt-3">
                  <button onClick={() => setOpen(isOpen ? null : s.id)} className="text-terracotta text-xs font-semibold hover:underline">
                        ...
                        <div key={m.id} className="border-oat/60 flex items-center justify-between gap-2 border-b py-1.5 text-sm last:border-0">
                          <span className="min-w-0 truncate">
                            ...
                            <span className="text-charcoal/50">
                              {m.reason || (m.type === "PAYIN" ? "Pay in" : "Pay out")} · {m.staffName}
                            </span>
                          </span>
                          <span className="text-charcoal/40 shrink-0 text-xs">{formatDateTime(m.createdAt)}</span>
                        </div>
                      ))}
                    </div>
                  )}
