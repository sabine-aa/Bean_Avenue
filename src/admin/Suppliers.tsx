...
export function AdminSuppliers() {
  ...
  const load = () => api.get<Supplier[]>("/api/suppliers?all=1").then(setSuppliers).catch(() => {});
  useEffect(() => { load(); }, []);
  ...
  return (
    <div>
      <h1 className="font-display text-3xl font-bold text-espresso">Suppliers</h1>
      <p className="mt-1 text-sm text-charcoal/60">The suppliers you buy stock from. Pick them when recording a restock.</p>
      ...
      <form onSubmit={add} className="mt-5 grid gap-2 rounded-2xl bg-white p-4 shadow-sm sm:grid-cols-3">
        ...
        <input value={form.contactPerson} onChange={(e) => setForm({ ...form, contactPerson: e.target.value })} placeholder="Contact person" className={field} />
        ...
        <button type="submit" className="rounded-full bg-terracotta px-5 py-2 font-semibold text-cream hover:bg-terracotta-dark">+ Add supplier</button>
      </form>
      ...
      <div className="mt-4 space-y-2">
          ...
          <div key={s.id} className={`rounded-2xl bg-white p-4 shadow-sm ${s.isActive ? "" : "opacity-50"}`}>
            {editingId === s.id ? (
              <div className="grid gap-2 sm:grid-cols-3">
                ...
                <input value={editForm.contactPerson} onChange={(e) => setEditForm({ ...editForm, contactPerson: e.target.value })} placeholder="Contact person" className={field} />
                <input value={editForm.phone} onChange={(e) => setEditForm({ ...editForm, phone: e.target.value })} placeholder="Phone" className={field} />
                <input value={editForm.whatsapp} onChange={(e) => setEditForm({ ...editForm, whatsapp: e.target.value })} placeholder="WhatsApp" className={field} />
                <input value={editForm.email} onChange={(e) => setEditForm({ ...editForm, email: e.target.value })} placeholder="Email" className={field} />
                <input value={editForm.address} onChange={(e) => setEditForm({ ...editForm, address: e.target.value })} placeholder="Address" className={field} />
                <input value={editForm.notes} onChange={(e) => setEditForm({ ...editForm, notes: e.target.value })} placeholder="Notes" className={`${field} sm:col-span-2`} />
                <div className="flex gap-2">
                  <button onClick={() => saveEdit(s.id)} className="rounded-full bg-terracotta px-4 py-1.5 text-sm font-semibold text-cream">Save</button>
                  <button onClick={() => setEditingId(null)} className="rounded-full bg-oat px-4 py-1.5 text-sm font-semibold">Cancel</button>
                </div>
              </div>
            ) : (
              <div className="flex flex-wrap items-center gap-3">
                <div className="flex-1">
                  <p className="font-semibold text-espresso">{s.name} {!s.isActive && <span className="ml-1 text-xs text-charcoal/40">(inactive)</span>}</p>
                  <p className="text-xs text-charcoal/55">
                    {[s.contactPerson, s.phone && `📞 ${s.phone}`, s.whatsapp && `💬 ${s.whatsapp}`, s.email].filter(Boolean).join(" · ") || "No contact details"}
                  </p>
                  {(s.address || s.notes) && <p className="mt-0.5 text-xs text-charcoal/45">{[s.address, s.notes].filter(Boolean).join(" · ")}</p>}
                </div>
        ...
        {suppliers.length === 0 && <p className="rounded-2xl bg-white p-6 text-center text-charcoal/50 shadow-sm">No suppliers yet — add one above.</p>}
      </div>
    </div>
  );
}
