...
export function AdminFeatured() {
  ...
  const load = () =>
    api
      .get<AdminData>("/api/featured/admin")
      .then((d) => {
        setData(d);
  ...
  return (
    <div>
      <h1 className="font-display text-espresso text-3xl font-bold">Homepage Featured Menu Items</h1>
      <p className="text-charcoal/60 mt-1 text-sm">
        Pick <span className="font-semibold">one product per category</span> for the “{title}” carousel — so it always shows variety.
      </p>
      ...
      <div className="mt-4 flex flex-wrap items-end gap-3 rounded-2xl bg-white p-4 shadow-sm">
        <label className="text-espresso text-sm font-semibold">
          Section title
          <input value={title} onChange={(e) => setTitle(e.target.value)} className="border-oat mt-1 block w-56 rounded-xl border px-3 py-2 font-normal" />
        </label>
        ...
        <button onClick={saveSettings} className="bg-espresso text-cream hover:bg-mocha rounded-full px-5 py-2 text-sm font-semibold">
          Save
        </button>
      </div>
      ...
      <div className="mt-4 overflow-hidden rounded-2xl bg-white shadow-sm">
        <div className="border-oat text-charcoal/45 hidden grid-cols-[1.4fr,2fr,auto] gap-3 border-b px-4 py-2 text-xs font-bold tracking-wide uppercase sm:grid">
          <span>Category</span>
          <span>Featured item</span>
          <span>Order · show · remove</span>
        </div>
        {data.rows.map((r, i) => {
          ...
          return (
            <div
              key={r.category}
              className={`border-oat/60 grid grid-cols-1 items-center gap-3 border-b px-4 py-3 sm:grid-cols-[1.4fr,2fr,auto] ${r.isHidden ? "opacity-50" : ""}`}
            >
              <span className="text-espresso font-semibold">{r.category}</span>
              <div className="flex items-center gap-2">
                <Img src={r.menuItem.photo} alt="" className="bg-oat/30 h-10 w-10 shrink-0 rounded-lg" />
                <select
                  value={r.menuItemId}
                  onChange={(e) => setProduct(r.category, Number(e.target.value))}
              ...
              <div className="flex items-center gap-1 justify-self-start sm:justify-self-end">
                <button onClick={() => move(i, -1)} disabled={i === 0} className="bg-oat rounded-full px-2 py-1 text-xs font-bold disabled:opacity-30">
                  ↑
                </button>
                <button
                  onClick={() => move(i, 1)}
                  disabled={i === data.rows.length - 1}
                  className="bg-oat rounded-full px-2 py-1 text-xs font-bold disabled:opacity-30"
                >
                  ↓
                </button>
                <button
                  onClick={() => toggleHide(r.category, !r.isHidden)}
                  className="bg-oat hover:bg-espresso hover:text-cream rounded-full px-3 py-1 text-xs font-semibold"
                >
                  {r.isHidden ? "Show" : "Hide"}
                </button>
                <button onClick={() => remove(r.category)} className="text-charcoal/40 hover:text-terracotta rounded-full px-2 py-1 text-xs font-semibold">
                  ✕
                </button>
              </div>
            </div>
        ...
        {data.rows.length === 0 && <p className="text-charcoal/50 p-6 text-center">No categories featured yet — add one below.</p>}
      </div>

      {/* Add a category */}
      {available.length > 0 && (
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <select value={addCat} onChange={(e) => setAddCat(e.target.value)} className="border-oat rounded-xl border px-3 py-2 text-sm">
          ...
          <button
            onClick={addCategory}
            disabled={!addCat}
            className="bg-terracotta text-cream hover:bg-terracotta-dark rounded-full px-4 py-2 text-sm font-semibold disabled:opacity-40"
          >
            Add
          </button>
        </div>
