...
export function AdminRewards() {
  ...
  return (
    <div>
      <h1 className="font-display text-3xl font-bold text-espresso">Rewards</h1>
      <p className="mt-1 text-sm text-charcoal/60">
        Rewards are organised by menu category. Add as many as you like per category and change the
        bean cost anytime.
      </p>
      ...
      <div className="mt-5 grid gap-6 lg:grid-cols-3">
          ...
          {grouped.map(({ cat, list }) => (
            <div key={cat}>
              <div className="flex items-center justify-between">
                <h2 className="font-display text-lg font-bold text-espresso">{cat}</h2>
                <button
              ...
              <div className="mt-2 space-y-2">
                {list.map((r) => (
                  <div
                    key={r.id}
                    className={`flex gap-3 rounded-2xl bg-white p-3 shadow-sm ${r.isActive ? "" : "opacity-60"}`}
                  >
                    <Img src={r.image} alt={r.name} className="h-16 w-16 shrink-0 rounded-xl" />
                    <div className="flex-1">
                      <p className="font-semibold text-espresso">
                        {r.name}
                        {!r.isActive && <span className="ml-2 text-xs text-terracotta-dark">hidden</span>}
                        {!r.isAvailable && (
                          <span className="ml-2 text-xs text-terracotta-dark">unavailable</span>
                        )}
                      </p>
                      ...
                      {r.description && (
                        <p className="line-clamp-1 text-xs text-charcoal/50">{r.description}</p>
                      )}
                    </div>
                    <div className="flex shrink-0 flex-col gap-1 text-xs">
                      <button onClick={() => startEdit(r)} className="rounded-full bg-oat px-3 py-1 font-semibold hover:bg-espresso hover:text-cream">
                        Edit
                      </button>
                      <button onClick={() => patch(r, { isActive: !r.isActive })} className="rounded-full bg-oat px-3 py-1 font-semibold hover:bg-espresso hover:text-cream">
                        {r.isActive ? "Hide" : "Show"}
                      </button>
                      <button onClick={() => patch(r, { isAvailable: !r.isAvailable })} className="rounded-full bg-oat px-3 py-1 font-semibold hover:bg-espresso hover:text-cream">
                        {r.isAvailable ? "Set N/A" : "Set avail"}
                      </button>
                      <button onClick={() => remove(r)} className="rounded-full bg-terracotta/15 px-3 py-1 font-semibold text-terracotta-dark hover:bg-terracotta hover:text-cream">
                        Delete
                      </button>
                    </div>
                  </div>
        ...
        <form onSubmit={save} className="h-fit rounded-2xl bg-white p-5 shadow-sm lg:sticky lg:top-6">
          ...
          <label className="mt-2 flex items-center gap-2 text-xs font-semibold text-espresso">
          ...
          <div className="mt-4 flex gap-2">
            ...
            {editingId && (
              <button type="button" onClick={() => startNew(form.category)} className="rounded-full bg-oat px-4 py-2 text-sm font-semibold">
                Cancel
              </button>
            )}
          </div>
